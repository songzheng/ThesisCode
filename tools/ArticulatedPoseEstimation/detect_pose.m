function boxes = detect_pose(im, model, thresh, det_scale)
...
if ~exist('det_scale', 'var')
    det_scale = [0, inf];
end

det_scale(1) = det_scale(1)/max(model.maxsize);
det_scale(2) = det_scale(2)/min(model.maxsize);

det_scale = model.sbin./det_scale([2,1]);

pyra     = featpyramid_pose(im,model, det_scale);
...
boxes = boxes(1:cnt,:);
[ignore, si] = sort(boxes(:,end), 'descend');
boxes = boxes(si, :);
