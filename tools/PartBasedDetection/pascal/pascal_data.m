...
try
    load([datadir cls '_', VOCopts.trainset,'_' year]);
catch
    % positive examples from train+val
    ids = textread(sprintf(VOCopts.imgsetpath, VOCopts.trainset), '%s');
    pos = [];
    numpos = 0;
    fprintf('%s: parsing positives, total %d\n', cls, length(ids));
    for i = 1:length(ids)
        if mod(i,round(length(ids)/20))==0
            fprintf('.');
        end
        ...
        for j = clsinds(:)'
            ...
            if flippedpos
                ...
                pos(numpos).trunc = rec.objects(j).truncated;
                if strcmp(rec.objects(j).view, 'Left')
                    pos(numpos).view = 'Right';
                elseif strcmp(rec.objects(j).view, 'Right')
                    pos(numpos).view = 'Left';
                else
                    pos(numpos).view = rec.objects(j).view;
                end
            end
    ...
    fprintf('%s: parsing negatives, total %d\n', cls, length(ids));
    for i = 1:length(ids);
        if mod(i,round(length(ids)/20))==0
            fprintf('.');
        end
    ...
    fprintf('\n');
    save([datadir cls '_',VOCopts.trainset,'_' year], 'pos', 'neg');
end
