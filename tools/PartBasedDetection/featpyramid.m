...
for i = 1:interval
    ...
    % "first" 2x interval
    pyra.feat{i} = model.features(scaled, sbin/2);
    pyra.scales(i) = 2/sc^(i-1);
    % "second" 2x interval
    pyra.feat{i+interval} = model.features(scaled, sbin);
    pyra.scales(i+interval) = 1/sc^(i-1);
    % remaining interals
    for j = i+interval:interval:max_scale
        scaled = resize(scaled, 0.5);
        pyra.feat{j+interval} = model.features(scaled, sbin);
        pyra.scales(j+interval) = 0.5 * pyra.scales(j);
    end
end

feat_dim = size(pyra.feat{1},3);
...
for i = 1:length(pyra.feat)
    ...
    pyra.feat{i}(1:pady+1, :, feat_dim+1) = 1;
    pyra.feat{i}(end-pady:end, :, feat_dim+1) = 1;
    pyra.feat{i}(:, 1:padx+1, feat_dim+1) = 1;
    pyra.feat{i}(:, end-padx:end, feat_dim+1) = 1;
end
pyra.padx = padx;
pyra.pady = pady;
