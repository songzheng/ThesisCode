...
for t = 1:iter
    ...
    for tneg = 1:negiter
        ...
        if isunix
            cmd = sprintf('%s %.6f %.6f %s %s %s %s %s %s %s %s %s', SVMpath,...
        ...
        if status ~= 0
            ...
            error('learn.out execution error');
        end
...
minsize = prod(pixels);
num = 0;
fprintf('%s %s: iter %d: warped positive, total %d\n', procid(), name, t, numpos);
for i = 1:numpos
    if mod(i,round(numpos/20))==0
        fprintf('.');
    end
    ...
    % get example
    im = warped{i};
    feat = model.features(im, model.sbin);
    feat(:,:,end+1) = 0;
...
scores = [];
fprintf('%s %s: iter %d/%d: latent positive, total %d\n', procid(), name, t, iter, numpos);
for i = 1:numpos
    if mod(i,round(numpos/20))==0
        fprintf('.');
    end
    ...
    %             fprintf(' (no overlap)\n');
    %         end
    bs = gdetectwrite(data.pyra, model, data.bs, data.info, 1, fid, i);
    if ~isempty(bs)
...
fprintf('%s %s: iter %d/%d: hard negatives, total %d\n', procid(), name, t, negiter, numneg);
  
for i = 1:numneg
    data = [];
    if mod(i,round(numneg/20))==0
        fprintf('.');
    end
    j = inds(i);
    ...
    bs = gdetectwrite(data.pyra, model, data.bs, data.info, ...
...
obl = model.rules{model.start}.offset.blocklabel;
num = 0;
fprintf('%s %s: iter %d: random negatives, total %d\n', procid(), name, t, numneg);
for i = 1:numneg
    if mod(i,round(numneg/20))==0
        fprintf('.');
    end
    im = imreadx(neg(i));
    feat = model.features(double(im), model.sbin);
