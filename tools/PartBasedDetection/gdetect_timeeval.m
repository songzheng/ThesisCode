function [dets, boxes, info] = gdetect_timeeval(pyra, model, thresh, bbox, overlap)
...
% cache filter response, compute initial score of each terminal shape model
fprintf('Filter convolution...\n');
tic;
model = filterresponses_gpu(model, pyra, latent, bbox, overlap);
toc;

% compute parse scores
fprintf('Compute final scores...\n');
tic;
L = model_sort(model);
for s = L
  for r = model.rules{s}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% compute all filter responses (filter score pyramids) using GPU
function model = filterresponses_gpu(model, pyra, latent, bbox, overlap)
% model    object model
% pyra     feature pyramid
% latent   true => latent positive detection mode
% bbox     ground truth bbox
...
% gather filters from precomputed fft models
filters_ptr = model.gpu.filters_ptr;
filters_size = model.gpu.filters_size;
fftH = model.gpu.fftH;
fftW = model.gpu.fftW;
filter_to_symbol = model.gpu.filter_to_symbol;
...
for level = levels
  % compute filter response for all filters at this level
  r = fconv_fft(pyra.feat{level}, filters_ptr, filters_size, fftH, fftW);
  % find max response array size for this level
  s = [-inf -inf];
  for i = 1:length(r)
    s = max([s; size(r{i})]);
